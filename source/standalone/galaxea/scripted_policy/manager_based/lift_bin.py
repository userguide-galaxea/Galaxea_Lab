...
from omni.isaac.lab_tasks.galaxea.manager_based.lift.lift_env_cfg import (
    LiftEnvCfg,
)
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            left_object_offset[:, 1] = 0.145
            left_object_offset[:, 2] = 0.025
            ...
            right_object_offset[:, 1] = -0.145
            right_object_offset[:, 2] = 0.025
            ...
            r_orientation_offset_euler[:, 0] = object_orientation_yaw
            ...
            l_orientation_offset_euler[:, 0] = object_orientation_yaw
            ...
            # print("Received shape of rgb   image: {}, dtype: {}".format(rgb_img.shape, rgb_img.dtype))
            # print("Received shape of depth image: {}, dtype: {}".format(depth_img.shape, depth_img.dtype))
            # print("rgb shape size: ", len(rgb_img.shape))
            # print("depth shape size: ", len(depth_img.shape))
            # print("depth max value: ", depth_img.max())   # tensor.min() >= 0 and tensor.max() <= 1
            # print("depth min value: ", depth_img.min())

            # rgb_img default dtype is uint8, convert to float since torch-vision only supports float
            rgb_img = rgb_img[:, :, :, :3].float() / 255.0
            depth_img = depth_img.permute(0, 2, 1) / 10.0
            if args_cli.save_data and count % 10 == 0:
                save_images_to_file(rgb_img, f"./data/lift_bin_rgb_{count}.png")
                save_images_to_file(depth_img, f"./data/lift_bin_depth_{count}.png")

            # # reset state machine
            if dones.any():
                print("[DEBUG] lift cube done...")
                print("[DEBUG] dones: ", dones)
                print("[DEBUG] reset_idx: ", dones.nonzero(as_tuple=False).squeeze(-1))
